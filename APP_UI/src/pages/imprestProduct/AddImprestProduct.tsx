...
import { createImprestProduct, getProducts } from "../../services/imprestProductService";
...
const AddImprestProduct = ({
    ...
    const handleSubmit = async () => {
        ...
        if (isValid) {
            try {
                const created = await createImprestProduct(imprestProducts);
                if (created) {
                    toast.success("Successfully added Imprest Product");
                    handleClose();
                } else {
                    toast.error("Product already exists in selected Imprest");
                    handleClose();
                }
            } catch (error) {
                toast.error("Failed to add Imprest Product");
                handleClose();
            }
        }
    };
                ...
                <div className="col-md-6">
                    ...
                    <SelectWithSearch
                        ...
                        onSelect={(e: any) => {
                            ...
                            setErrors((prev) => ({ ...prev, imprest_id: "" }));
                        }}
                    />
